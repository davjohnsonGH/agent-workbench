...
Evals (paid API calls; never in CI; see `evals/README.md`): `npm run eval:pm -- --only todo-app` (after a one-time `--approve-harness`).
...
npm-workspaces + Turborepo monorepo (`apps/*`, `packages/*`, `evals`):
...
- `evals` — offline quality evals that call the real agent code (`buildAgentRequest` + `AnthropicProvider`) over fixed cases: `lib/runner.ts` (resume by (case, rep), per-case timeout, jittered retries with attempt counts, `errors.jsonl` for unscorable attempts, Wilson CIs, and a harness-integrity gate that refuses to run after eval code changes until re-approved with `--approve-harness`, which is the user's call) and `pm/` (cases, judge + programmatic graders). Output: `.claude/hillclimb/<flow>/<variant>/`. Changing `evals/pm/*` or `evals/lib/*` invalidates the approval.
